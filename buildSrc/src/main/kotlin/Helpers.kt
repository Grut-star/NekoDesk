...
fun Project.requireMetadata(): Properties {
    if (!::metadata.isInitialized) {
        metadata = Properties().apply {
            load(rootProject.file("nd4a.properties").inputStream())
        }
    }
    return metadata
}
            ...
            outputs.all {
                ...
                outputFileName = if (isPreview) {
                    outputFileName.replace(
                        project.name,
                        "NekoDesk-" + requireMetadata().getProperty("PRE_VERSION_NAME")
                    ).replace("-preview", "")
                } else {
                    outputFileName.replace(project.name, "NekoDesk-$versionName")
                        .replace("-release", "")
                        .replace("-oss", "")
                }
            }
